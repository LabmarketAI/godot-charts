...
func request_layout(available_size: Vector2 = Vector2.ZERO) -> void:
	...
	var cursor_x: float = -available_size.x * 0.5 + padding.x
	var top_y: float = available_size.y * 0.5 - padding.y

	for child in children:
		var child_size := _get_child_widget_size(child)
		child.position = Vector3(cursor_x + child_size.x * 0.5, top_y - child_size.y * 0.5, 0.0)
...
func _measure_content_size() -> Vector2:
	var children := _iter_widget_children()
	var total_w: float = padding.x * 2.0
	var max_h: float = 0.0
	...
	return Vector2(max(total_w, padding.x * 2.0), max_h + padding.y * 2.0)
