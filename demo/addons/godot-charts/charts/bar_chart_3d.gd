...
## Per-dataset texture overrides.  Index 0 → first dataset, index 1 → second, etc.
## When provided, the texture is applied to the material's albedo.
## Requires [member bar_materials] to be empty or use [StandardMaterial3D] for the texture to be visible.
...
func _render_bar_data(d: Dictionary) -> void:
	...
	for ds_idx in n_datasets:
		...
		var texture: Texture2D = bar_textures[ds_idx] if ds_idx < bar_textures.size() else null
		var mat: Material = _create_material_with_texture(color, texture, override)
