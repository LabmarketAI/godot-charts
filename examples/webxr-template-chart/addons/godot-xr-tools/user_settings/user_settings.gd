...
func get_adjusted_vector2(p_controller, p_input_action):
	...
	if p_input_action == "primary" and original_vector.is_zero_approx() and XRServer.find_interface("WebXR"):
		var fallback_action = get_webxr_primary_action(get_real_webxr_primary())
		if fallback_action != "auto":
			original_vector = p_controller.get_vector2(fallback_action)
		if original_vector.is_zero_approx():
			original_vector = p_controller.get_vector2("thumbstick")
		if original_vector.is_zero_approx():
			original_vector = p_controller.get_vector2("trackpad")
		if original_vector.is_zero_approx():
			original_vector = p_controller.get_vector2("secondary")
