...
| Python companion preview | Matplotlib 3.10–3.11, pandas 2.2–3.0, `websockets` 15–16 | Adopt behind companion contracts | `python/pyproject.toml` declares the separately installed Python service boundary. The first adapter accepts an explicit Figure, DataFrame, stable index, and mappings; it emits bounded JSON only and rejects arbitrary object scalars. Python remains optional and outside the Godot addon/runtime package. |
| Message bus/WebSocket | Godot 4.6.3 built-in `WebSocketPeer`; Python `websockets` 16.0 fixture publisher | Wrap as optional integration | The first live bridge lives under `integrations/` and feeds the same replay-tested envelopes into the protocol consumer. Built-in Godot support avoids another runtime dependency and is Web-export compatible. The localhost test publisher pins `websockets==16.0` separately and is not shipped runtime code. Credentials remain outside core and transport traces redact URL paths, queries, and user information. |
