...
Status: initial decisions for the architectural spine, 2026-07-16. Versions are to be pinned in manifests after the standard-Godot harness proves compatibility.
...
| Python fixture generation | Matplotlib, pandas, and Python JSON Schema tooling | Adopt as fixture-only dependencies | They run outside Godot to create deterministic checked-in fixtures. No Python runtime is required by the addon or demo replay. Pin supported versions in the future companion package. |
...
The runtime addon has no approved mandatory third-party dependency for M1. GdUnit4 and Python fixture packages are development-only and must be declared with versions, licenses, purpose, and installation instructions when introduced. Optional integrations must live outside core, advertise capabilities at runtime, fail transparently, and leave the recorded demo functional when absent.
