...
Core classes extend `RefCounted` only. Renderer and table adapters receive core models through public methods. Interaction state uses stable row identities and duck-typed ports so it has no renderer or input-device import. The generated preview allowlist and `check-m1-boundaries.sh` enforce these rules in CI.
