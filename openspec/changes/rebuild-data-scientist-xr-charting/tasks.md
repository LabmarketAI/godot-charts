...
- [x] M1.1 Complete the legacy audit and dependency scorecards needed by this slice; record keep/adapt/rewrite/remove and adopt/wrap/optional/build-minimal decisions.
  - Evidence: `legacy-audit.md`, `legacy-public-surface.md`, and `dependency-scorecard.md` inventory registered families, C#/GDScript/tooling/demo consumers, record subsystem and package dispositions, define the clean-break/conditional-adapter policy, pin M1 dependencies, and record product-owner acceptance through the exit review.
- [x] M1.2 Establish clean pure-model, Godot-renderer, input-intent, protocol/replay, and optional-integration boundaries plus a standard-Godot CI fixture with no .NET or native runtime dependency.
  - Evidence: `m1-architecture.md` defines ownership and dependency direction; `check-m1-boundaries.sh` rejects core scene coupling, reverse imports, and legacy dependencies. The generated preview and official-Godot CI prove the boundaries without .NET, native binaries, demo code, network, or optional integrations.
...
- [x] M1.12 Publish the minimal quickstart and reference scene from released public APIs, record measured results and remaining risks, and require an explicit M1 review before live transport, Jupyter authentication, additional chart types, or WebXR implementation begins.
  - Evidence: `examples/m1/`, `scripts/prepare-m1-example.sh`, and `m1-review.md` provide the self-contained public reference scene, five-minute preparation path, measurements, deferred risks, and review boundary. Official-Godot CI run `29549601510` passed the scene; product-owner continuation accepted the review boundary before M1 closure.
...
- [x] M2.1 Audit the legacy `ChartFrame3D`, frame orchestration, placement/move flows, workspace persistence, slider/pointer controls, and XR grab behavior as migration evidence; record reusable behavior and coupling failures without importing legacy or demo code.
  - Evidence: `m2-legacy-frame-audit.md` records preserved behavior and the C# wrapper, destructive rebuild, bottom-left-origin compensation, demo-orchestrator, timestamp identity, child-type inference, immediate persistence, device branching, and unschematized-workspace failures. It explicitly prohibits importing the audited surfaces.
- [x] M2.2 Define pure typed-GDScript frame state and binding ports with stable frame identity, transform, bounds/aspect, title/source/status chrome, theme reference, visibility/lock state, authored reset state, and local view state. Frame state SHALL not own source data, replay, transport, or renderer internals.
  - Evidence: `frames/analytical_frame_state.gd` and `frames/frame_binding.gd` are scene-independent `RefCounted` contracts with deterministic serialization, validation diagnostics, current/authored transforms, 3D bounds, status/theme/visibility/lock/local-view state, transport-neutral source identity and revision, and representation policy. The clean M2 harness proves exact round trips and reset behavior; the boundary audit rejects scene, protocol, presentation, integration, device, and legacy dependencies.
- [x] M2.3 Implement a retained `AnalyticalFrame3D` presentation node that hosts the existing renderer and bounded table through public ports, preserves content resources across compatible plot revisions, and remains installable without legacy charts, widgets, demo services, .NET, native binaries, or XR packages.
  - Evidence: `renderers/analytical_frame_3d.gd` projects validated frame state atomically into stable `ContentRoot`, `GuideRoot`, `ChromeRoot`, `HandleRoot`, bounds mesh, transform, visibility, lock/status/theme chrome ports, and deterministic preserve/fit/free content sizing. It owns the scatter renderer while retaining an external bounded-table port and no data, replay, persistence, transport, or device policy. The M2 harness proves invalid-state last-good behavior, stable frame/content nodes, compatible revision and linked-selection preservation, and no node or `MultiMesh` replacement across repeated state applications.
- [x] M2.4 Implement deterministic linear XYZ ticks, grid lines, axes, tick labels, axis labels, title, and interior orientation/reset landmarks from retained scale/guide state, with missing/degenerate-domain behavior and no decorative geometry that changes perceived values.
  - Evidence: `core/linear_ticks.gd` generates bounded nice-number ticks and deterministic fixed/scientific formatting. `renderers/cartesian_guides_3d.gd` retains axis/grid `ImmediateMesh` resources and pooled billboard labels under the frame's guide/chrome roots, deriving XYZ positions, titles, domain labels, figure title, positive-axis landmarks, and an origin/reset landmark solely from retained scales/guides and fitted plot bounds. M2 tests cover exact year/enrollment ticks, formatting, degenerate-domain rejection, source titles, populated line surfaces, compatible-revision identity, stable label pools, and no lifecycle growth.
- [x] M2.5 Define explicit `content`, `frame`, and `navigate` input modes with visible ownership/capture. Content mode SHALL route inspection and row selection; frame mode SHALL route whole-frame manipulation; navigate mode SHALL change observer/view orientation without mutating frame transform or analytical scale domains.
  - Evidence: `interactions/frame_interaction_controller.gd` exposes deterministic intent ownership for content inspection/selection, whole-frame operations, and navigation. Mode changes cancel active capture and restore exact state. `AnalyticalFrame3D` projects selected-frame, active-capture, navigate, and neutral states through distinct retained bounds colors and handle-root visibility. Tests prove wrong-mode rejection, explicit selection, visible capture, focus/mode restoration, and navigation updates that preserve frame transform and plot bounds.
- [x] M2.6 Implement device-independent frame commands for select, begin manipulation, preview move/rotate/resize, commit, cancel, lock/unlock, undo, redo, and reset. Commands SHALL use stable identities, preserve exact before/after state, reject invalid transitions atomically, and remain independent of mouse, OpenXR, WebXR, or XR Tools APIs.
  - Evidence: `frame_interaction_controller.gd` implements owner-scoped begin/preview/commit/cancel transactions, absolute capture-origin previews, atomic bounds validation, reversible lock/unlock and reset, exact snapshot history, undo/redo branch truncation, configurable bounded eviction, and deterministic command traces. `AnalyticalFrameState.replace_from` preserves object identity during restore. Headless tests cover non-owner and locked rejection, move/rotate/resize, invalid resize, exact cancel, capture loss, authored reset, navigation preservation, undo/redo, branching, limits, and event traces with no device imports.
- [x] M2.7 Implement a desktop mouse/keyboard adapter and a deterministic mocked-XR ray/grab adapter that emit the same intents and command sequences, including capture loss, cancellation, mode switching, and focus restoration.
  - Evidence: `interactions/frame_input_intent_router.gd` owns the shared semantic intent mapping while `desktop_frame_input_adapter.gd` and `mock_xr_frame_input_adapter.gd` remain thin `RefCounted` translators with no engine input, OpenXR, WebXR, or XR Tools imports. The official-Godot M2 harness drives equivalent mouse/keyboard and ray/grab workflows through move commit, resize preview, focus/tracking loss cancellation, navigation, frame-focus restoration, undo, and redo; it proves byte-equivalent serialized frame state and exact controller command-trace parity.
- [x] M2.8 Extend the public reference scene so a user can read the scatter's title and XYZ guides, inspect/select a row, move and resize its frame, receive a compatible replay or live replacement, undo/cancel manipulation, and reset to authored state without consulting fixture JSON.
  - Evidence: the self-contained `examples/m1/` reference composes the public retained frame, Cartesian guides, scatter, table, controller, and desktop adapter APIs. It pauses at revision 1 and uses global key handling plus a persistent visible legend so table focus cannot hide controls. `1`/`2`/`3` select content/frame/navigate modes; navigate mode moves the observer with WASD and Q/E without changing frame or scale state; frame mode exposes select and exact move/resize begin-preview-commit/cancel transactions, undo/redo, authored reset, and stepwise compatible replacement. Live status reports revision, mode, selection, capture, and rendered points. `test-m1-example.sh` prepares and launches this scene from the clean allowlisted addon under official standard Godot.
- [x] M2.9 Add headless interaction replay, numerical guide, lifecycle, and package tests proving desktop/mocked-XR parity; exact cancel/undo/reset; frame/content/navigation disambiguation; preservation of frame transform, local view, eligible selection, and picking across revision/reconnect; and no scene-tree or rendering-resource growth under repeated updates and manipulations.
  - Evidence: the official-Godot M2 harness combines exact numerical tick/guide assertions, mode and restoration transitions, identical desktop/mocked-XR recorded workflows, and 100 repeated manipulation transactions with exact serialized-state/trace parity, bounded history, and unchanged lifecycle snapshots. The live WebSocket fixture performs two complete handshakes with one forced automatic reconnect through `AnalyticalFrame3D`; it proves pre-delivery user transform/local-view state, eligible linked selection, stable picking, frame roots, render node, and `MultiMesh` survive both compatible revision and reconnect. The allowlisted package/editor gates run before every headless suite.
- [x] M2.10 Publish an M2 review with visual/readability evidence, measured desktop budgets, remaining accessibility/XR risks, dependency decisions, and an explicit gate before selecting production WebXR wiring, Jupyter discovery/authentication, or additional chart families.
  - Evidence: `m2-review.md` records the public-scene and automated guide/readability evidence, measured headless content and paired interaction budgets, stable lifecycle results, dependency decisions, and unresolved human accessibility, graphical desktop, native-XR, and WebXR risks. It closes the bounded M2 architecture while explicitly withholding authorization for production XR wiring, Jupyter discovery/authentication, and additional chart families until the product owner selects and scores one next slice.
...
## Milestone M3: WebXR-First Delivery Foundation

The product owner has selected WebXR as the primary delivery target, with a simple non-immersive web mode from the same Godot build as the required fallback. `specs/webxr-first-delivery/spec.md` is the governing delivery contract.

- [x] M3.1 Define the WebXR-first and flat-web shared-build contract, secure hosting boundary, baseline capability negotiation, accessibility fallback, and browser/headset release matrix.
  - Evidence: `specs/webxr-first-delivery/spec.md` requires one shared semantic application, explicit immersive entry and loss recovery, ray/select baseline, non-XR keyboard/pointer fallback, HTTPS/WSS production hosting, and separate export versus runtime-certification evidence.
- [x] M3.2 Add a committed standard-Godot Web export preset and reproducible clean-example build gate using Compatibility/WebGL 2.0, single-threaded export, no extension support, and required HTML/JavaScript/WebAssembly/PCK artifact checks.
  - Evidence: `examples/m1/export_presets.cfg` and `scripts/build-web-example.sh` prepare the allowlisted public example, invoke Godot's release exporter, verify all generated artifacts and the generated single-thread/empty-GDExtension configuration, and reject native libraries. An actual release export with official Godot 4.6.3 and its matching `web_nothreads_release.zip` completed on 2026-07-17, producing `index.html`, 309 KiB JavaScript, 36 MiB WebAssembly, and a 135 KiB pack. The build requires matching official templates and does not vendor them.
- [x] M3.3 Implement WebXR session lifecycle and runtime capability reporting with explicit enter/exit UI, supported reference-space selection, loss recovery, authored pose/scale, and return to flat-web mode without state loss.
  - Evidence: `integrations/webxr_session_controller.gd` wraps Godot's signal-driven WebXR interface, checks immersive support, requires `local-floor`, prefers bounded/local fallbacks, reports the selected reference space/features/errors, and restores non-XR rendering on exit or failure. The public scene adds an authored `XROrigin3D`/`XRCamera3D` and user-gesture Enter/Exit VR button while retaining the desktop camera and analytical state. `test-webxr-session.sh` proves unavailable, supported, asynchronous start, explicit exit, synchronous rejection, and asynchronous failure transitions without a browser dependency.
- [x] M3.4 Implement the real WebXR pose/ray/select adapter against the M2 intent/command contract, then add controller squeeze/grab and hand tracking only as progressive enhancements.
  - Evidence: `interactions/webxr_frame_input_adapter.gd` binds Godot's real WebXR `selectstart`/`selectend` and optional `squeezestart`/`squeezeend` signals, reads tracked aim poses and target-ray modes, focuses a frame through ray/select, previews device-independent movement from pose deltas, commits on release, and cancels on session failure/loss. The adapter requires no XR Tools package and makes squeeze an enhancement over the select baseline. The M3 suite proves exact serialized-state and command-trace parity with the mocked-XR adapter plus loss cancellation; the public scene polls poses only during an active immersive session.
- [x] M3.5 Make the flat-web presentation responsive and pointer-complete, with keyboard focus, textual inspection/status/help, WebXR availability explanation, and identical analytical/frame state transitions.
  - Evidence: the shared public scene now uses canvas-item stretch, anchored panels, and a narrow-viewport table layout. Its focusable pointer controls cover content/frame/navigation modes, select, move/resize preview, directional movement, size changes, commit/cancel, undo/redo/reset, and revision stepping while the keyboard workflow remains documented. Persistent text explains WebXR checking, availability, startup, active reference space, and fallback/failure states. `test-web-ui.sh` activates the pointer workflow and proves exact move/undo/redo/reset presentation state plus readable status.
- [x] M3.6 Add automated release-export inspection plus browser smoke coverage for startup, recorded replay, input, revision preservation, WebSocket/WSS failure fallback, session entry/exit, capability loss, and reload/cache behavior.
  - Evidence: `test-web-browser.sh` performs a clean release export, serves it with Python's MIME-aware static server, launches Brave headlessly, and drives the focused Godot canvas through raw Chrome DevTools Protocol without a browser automation package. `window.godotChartsSmoke` exposes bounded read-only state for assertions. Real Brave execution proves Wasm startup, recorded revision/point count, trusted keyboard move/commit input, WebXR-unavailable textual fallback, deterministic reload to the authored frame/revision, absence of browser/Godot fatal errors, and failed WSS fallback that retains the recorded four-point revision while reporting a redacted endpoint. The optional live probe uses a separate replay consumer so connection failure cannot clear displayed offline state. The run also caught and fixed release-only initialization calls that had incorrectly lived inside stripped `assert(...)` expressions. `test-webxr-session.sh` supplies deterministic supported-session entry/exit, start failure, and capability-loss recovery coverage that headless Brave cannot expose without an immersive runtime; real headset certification remains M3.7.
- [ ] M3.7 Publish and execute the first browser/headset matrix with exact versions, HTTPS hosting configuration, reference spaces, inputs, mono/stereo frame budgets, update latency, memory, data limits, thermal behavior, and known limitations before calling WebXR supported.
  - Partial evidence: `m3-release-matrix.md` records the exact Godot/preset/hosting contract, Brave/Linux flat-web tier, content limits, known limitations, and candidate 60/72/90 Hz budgets. The automated release smoke now samples 180 mono animation frames, command-to-published-state latency, and JavaScript heap. On the 2026-07-17 SwiftShader run it measured 16.7 ms median / 16.8 ms p95 frame interval, 49.2 ms median / 53.9 ms max across five command samples, and 17,336,956 used heap bytes. The matrix explicitly labels the build experimental; M3.7 remains open until a representative physical WebXR headset row records runtime/browser versions, reference space, ray/select workflow, stereo timing, memory, session loss/exit, and 15-minute thermal/degradation behavior.
...
- [x] 3A.2 Build the companion Python adapter package with registry, compatibility report, resource limits, deterministic IDs, and no-pickle/no-code security tests.
  - Evidence: the installable `godot-charts-companion` package provides explicit `AdapterRegistry` registration, structured supported/approximated/rejected `CompatibilityReport` results, SHA-256-based process-stable protocol IDs, bounded dataframe/message conversion, JSON-scalar allowlisting, and live publishing. Python tests reject duplicate identities, arbitrary object values, row/column overflow, duplicate adapters, incompatible figures, pickle-family/subprocess imports, and direct evaluation/compilation calls; official CI also proves the companion-to-Godot path.
...
- [ ] 4B.2 Define the quiet-scientific-instrument principles and semantic token schema for color, typography, dimensions, depth, materials, lighting, states, motion, target size, viewing distance, density, and effect budgets.
  - Progress: the first typed-GDScript token resource defines light and WebXR-performance color, dimension, interaction-state, reduced-motion, and per-asset budget tokens for procedural chart assets; typography, viewing-distance classes, density tiers, full effect budgets, and validated dark/high-contrast/color-vision-safe/presentation themes remain.
...
- [ ] 4B.4 Build reusable Godot assets for axes, ticks, grids, frames, points, bars, lines/tubes, surfaces, arrows, uncertainty marks, legends, annotations, and orientation landmarks with declared pivots, dimensions, batching, and LOD.
  - Progress: `addons/godot-charts/assets/visual/` now exposes procedural assets for axis/tick/grid lines, plot bounds, origin/reset landmarks, points, bars, lines, and fallback primitives with role metadata, dimensions, pivots, sockets, provenance, and WebXR budget descriptors. Surfaces, arrows, uncertainty marks, legends, annotations, richer LODs, and renderer-wide adoption remain.
- [ ] 4B.5 Build reusable interactive assets for hover/focus, handles, slider tracks/thumbs, dials, grab anchors, buttons, slice planes, thresholds, selection volumes, tooltips, and reset controls across ray and direct-touch modes.
  - Progress: the procedural visual layer now provides hover halos, redundant focus rings, linear handles, slider tracks/thumbs, grab anchors, buttons, and reset controls with collision proxies and non-color focus cues. The WebXR template chart frame handles consume these assets through the semantic role factory while preserving XR Tools grab/pointer behavior. Dials, slice planes, thresholds, selection volumes, tooltips, binding-specific parameter controls, and full ray/direct-touch validation remain.
...
- [ ] 4B.7 Create a visual-system gallery showing every asset, token, theme, state, viewing-distance class, density level, interior/exterior condition, and WebXR fallback from public APIs.
  - Progress: `VisualAssetGallery3D` and `examples/visual-assets/` instantiate every currently registered procedural role through public addon APIs, and `scripts/test-visual-assets.sh` loads the gallery scene in a clean temporary project. Full token/theme matrices, viewing-distance/density/interior variants, baseline screenshots, and WebXR fallback review remain.
...
- [ ] 4B.9 Define the versioned asset-pack manifest, semantic role registry, sockets/material parameters, inheritance/fallback chain, validation diagnostics, and compatibility migration policy.
  - Progress: the first `asset_pack_manifest.json`, `VisualAssetRoles`, and procedural factory descriptors define stable role ids, categories, sockets, dimensions, license/source provenance, WebXR per-asset budgets, and minimal fallback roles. Versioned migration policy, pack inheritance, structured validation diagnostics, and external pack compatibility checks remain.
