...
- [ ] M1.11 Prove deterministic replay, numerical/semantic assertions, public API usage, headless lifecycle, selection linkage, replacement behavior, resource stability, clean packaging, and desktop performance in CI.
  - Started: an allowlisted pure-GDScript preview artifact rejects C#/.NET/native/demo dependencies, loads as an editor plugin, and passes deterministic model/replay/render/table/session/diagnostics plus measured lifecycle budgets on official Godot 4.6.3. `.github/workflows/ci.yml` now runs the pinned Python and standard-Godot gates; remote CI confirmation remains open.
