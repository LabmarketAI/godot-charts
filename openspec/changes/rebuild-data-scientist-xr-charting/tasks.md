...
- [x] M2.8 Extend the public reference scene so a user can read the scatter's title and XYZ guides, inspect/select a row, move and resize its frame, receive a compatible replay or live replacement, undo/cancel manipulation, and reset to authored state without consulting fixture JSON.
  - Evidence: the self-contained `examples/m1/` reference now composes the public retained frame, Cartesian guides, scatter, table, controller, and desktop adapter APIs. It pauses at revision 1 and exposes labeled keyboard controls for content/frame/navigate modes, linked table selection, explicit move/resize begin-preview-commit/cancel transactions, undo/redo, authored reset, and stepwise compatible replacement; its live status reports revision, mode, selection, capture, and rendered points. `test-m1-example.sh` prepares and launches this scene from the clean allowlisted addon under official standard Godot.
